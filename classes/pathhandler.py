...
class PathHandler:
	...
	def check_source_path(self, src_path):
		...
		if not src_path.is_dir():
			return path, NotADirectoryError(self._labels.bad_source.replace('#', f'{src_path}'))
		if not self._re.match(src_path.name):
			return path, PermissionError(self._labels.bad_source.replace('#', f'{src_path}'))
		if path := self.search_trigger_file(src_path):
			return path, PermissionError(self._labels.bad_file.replace('#', f'{path}'))
		if path := self.search_too_long_path(src_path):
			return path, OSError(self._labels.path_too_long.replace('#', f'{path}'))
		if path := self.search_blacklisted(src_path):
			return path, Warning(self._labels.blacklisted_path.replace('#', f'{path}'))
